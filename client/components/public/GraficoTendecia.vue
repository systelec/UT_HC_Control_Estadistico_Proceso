  ...
  methods: {
    ...
    graficarTendencias() {
      ...
      if (this.codigoProductoSelecionado !== 'Todos') {
        producto = this.codigoProductoSelecionado
      }

      let historicos = this.historicosAcumulados.historicos.filter(
        historico => {
          if (!producto) {
            return true
          } else {
            if (parseInt(historico.codigo_producto) === parseInt(producto)) {
              return true
            }
            return false
          }
