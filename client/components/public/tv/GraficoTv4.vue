<template>
  <div>
    <img class="loading" v-if="loadingTv4Grafico" src="../../../static/charts.png" width="50">
...
export default {
  ...
  computed: {
    ...mapGetters([
      'variablesTv4TiempoReal',
      'variablesTv4Historicas',
      'actualizarTv4Grafico',
      'loadingTv4Grafico'
    ])
  },
  ...
  methods: {
    graficarTendencias() {
      let myChart = echarts.init(document.getElementById('GraficoTV4'))

      this.titulo = this.variablesTv4Historicas
        ? this.variablesTv4Historicas.Tendencia
        : ''

      this.fueraLimite = this.variablesTv4Historicas
        ? this.variablesTv4Historicas.Fuera_Limite
        : ''

      let intervalo = 1000000
      let limite = this.variablesTv4Historicas.Limite || null
      let media = 0
      let mediaR = 0
      let lsl = 0
      let usl = 0
      ...
      let hasta = moment(
        this.variablesTv4Historicas ? this.variablesTv4Historicas.Hasta : ''
      )
      let desde = moment(
        this.variablesTv4Historicas ? this.variablesTv4Historicas.Desde : ''
      ).add(-intervalo, 'seconds')

      let historicos = this.variablesTv4Historicas
        ? this.variablesTv4Historicas.Historicos
        : []
